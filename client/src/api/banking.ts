...
export const getAccountBalances = () => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      resolve({
        accounts: storedBalances[userEmail]
      });
    }, 500);
  });
...
export const transferMoney = (recipientEmail: string, amount: number) => {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      // Check if sender has sufficient funds (checking account balance)
      if (storedBalances[senderEmail].checking < amount) {
        reject(new Error('Insufficient funds in checking account'));
        return;
      }

      // Update balances
      storedBalances[senderEmail].checking -= amount;
      storedBalances[recipientEmail].checking += amount;
